...
void CMFCApplication1View::OnDraw(CDC* pDC/*pDC*/)
{
	...
	if (histFlag == 1)
	{
		...
		for (i = 0; i < 256; i++)
		{
			pDC->MoveTo(width + 30 + 1 * i, height + 50);
			pDC->LineTo(width + 30 + 1 * i, height + 50 - hist[i] / 6.);
		}
...
void CMFCApplication1View::OnFileOpen()
{
	...
	MyFDlg.m_ofn.lpstrInitialDir = "C:\\Users\\Administrator\\Downloads\\实验平台\\bb";
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++) {
			if (i == 0 || j == 0 || i == height - 1 || j == width - 1) {
				outImg[i * width + j] = image[i * width + j];
			}
			else {
				for (int m = -1; m < 2; m++) {
					for (int n = -1; n < 2; n++) {
						block[(m + 1) * 3 + (n + 1)] = image[i * width + j + (m * width) + n];
					}
				}

				outImg[i * width + j] = convolution(mask, block) / 9;
			}
		}
	}
}
...
int CMFCApplication1View::convolution(int* mask, BYTE* block)
{
	int value;
	int i, j;
	value = 0;
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++) {
			if (i == 0 || j == 0 || i == height - 1 || j == width - 1) {
				...
				outImg[i * width + j] = convolution(mask, block) / 16;
			}
		}
	}
}
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++) {
			if (i == 0 || j == 0 || i == height - 1 || j == width - 1) {
				...
				outImg[i * width + j] = MidValueFind(9, block);
	...
	for (int i = 0; i < hei; i++) {
		for (int j = 0; j < wid; j++) {
			if (i == 0 || j == 0 || i == hei - 1 || j == wid - 1) {
				...
				sob_x[i * wid + j] = convolution(so_x, block);
				sob_y[i * wid + j] = convolution(so_y, block);
...
void CMFCApplication1View::OnBmp()
{
	...
	if (MyFDlg.DoModal() == IDOK)
	{
		...
		char* fnstr;
		fnstr = (char*)SFName.GetBuffer(4); //read the name from string 
